...
export default function OnboardingScreen() {
  ...
  const handleComplete = async () => {
    ...
    try {
      console.log('[Onboarding] Completing registration for email:', normalizedEmail);
      await register({ name: trimmedName, email: normalizedEmail, password: sanitizedPassword });
      router.replace('/');
    } catch (error) {
      Alert.alert(
        'Registration failed',
      ...
      >
        ...
        >
          ...
          <View>
            ...
            onPress={goToLogin}
            activeOpacity={0.8}
            testID='goToLoginRedirect'>
              <Text style={styles.description}>Already have an account?
              </Text>
